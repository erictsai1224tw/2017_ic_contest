	...
	if (fwpass_finish)
	{
		...
		for (int i = 0; i < N_PAT; i++)
		{
			...
			else
			{
				fw_err = fw_err + 1;
				if (fw_err <= 30)
					cout << "FWPASS : Output pixel " << setw(5) << i << " are wrong! the real output is " << rel_pat << " , but expected result is " << exp_pat << endl;
				if (fw_err == 31)
					cout << "FWPASS : Find the wrong pixel reached a total of more than 30 !, Please check the code ..... " << endl;
			}
		...
		if (done)
		{
			...
			for (int i = 0; i < N_PAT; i++)
			{
				...
				else
				{
					bc_err = bc_err + 1;
					if (bc_err <= 30)
						cout << "Output pixel " << setw(5) << i << " are wrong!the real output is " << rel_pat << ",  but expected result is " << exp_pat << endl;
					if (bc_err == 31)
						cout << "Find the wrong pixel reached a total of more than 30 !, Please check the code ....." << endl;
				}
				if (((i % 1000) == 0) || (i == N_PAT - 1))
				{
					if (bc_err == 0)
